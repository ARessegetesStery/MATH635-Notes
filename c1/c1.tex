...
\begin{definition}[Riemannian Structure]
    ...
    \]
    where $\restr{\pp{}{x^i}}{p} \in M$ for all $i$. Then $g_{ij}$ is $C^{\infty}$; and $g(p) = (g_{ij}(p))$ is a symmetric matrix that depends on $p$. The matrix is often referred to as the \textbf{metric tensor}. The calculation can be defined via specifying $g_p(v_1, v_2) = v_1^T g(p) v_2$.
\end{definition}
