...
\begin{definition}[Normal Covering]
    A covering $\pi$ is \textbf{normal} if $\Aut(\pi)$ acts transitively as fibers. 
\end{definition}

\begin{proposition}
    Suppose a manifold $M$ has a Riemannian metric, and $f \in \Aut(\pi)$ is an isometry. Then there exists a unique Riemannian metric on $N$ s.t. $\pi$ is a local isometry.
\end{proposition}

\begin{proof}
    Fix $q \in N$. Choose $p \in \pi^{-1}(q)$. 
\end{proof}
