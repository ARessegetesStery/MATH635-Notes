...
\begin{definition}[Tensor Product]
    Let $V, W$ be finite dimensional vector space. The \textbf{tensor product of $V$ and $W$} $V \tensor W$ is the free vector space generated by $(V \times W)/ \sim$, where $\sim$ follows the rules:
    \begin{itemize}
...
\begin{remark}
    Not every element in $V \tensor W$ can be expressed as $v \tensor w$ for some $v \in V, w \in W$. In particular, one can have $v' = \sum_{i \in I} v_i \tensor w_i \in V \tensor W$ but whose representative cannot be further simplified. 
\end{remark}
...
\begin{definition}
    ...
    \]
    Then an element in $G$ is called a $(k, \ell)$-tensor on $V$. $k$ is the \underline{contravariant degree}, and $\ell$ is the \underline{covariant degree}.
\end{definition}
